...
const number_displayed = 2;
@registerGObjectClass
export class TranslationTest extends St.Widget {
    static metaInfo: GObject.MetaInfo = {
        GTypeName: 'TranslationTest',
    };

    vertical: boolean;
    childContainer = new St.Widget({
        style: 'background-color: rgba(0,100,0,0.5)',
        x_expand: true,
    });
    translationHelper = new TranslationHelper(this.childContainer, false);
    windowSimList: St.Widget[] = [];
    displayed = 0;
    ...
    constructor(vertical = false) {
        ...
        for (let index = 0; index < 6; index++) {
            const windowSim = new St.Widget({
                layout_manager: new Clutter.BinLayout(),
                style: `background-color: rgba(${10 * index},${
                    20 * index
                },100,0.5)`,
                x: 100 * index,
                width: 100,
            ...
            windowSim.add_child(new St.Label({ text: index.toString() }));
            this.windowSimList.push(windowSim);
        }
        this.childContainer.add_child(this.windowSimList[this.displayed]);

        for (let index = 1; index < number_displayed; index++) {
            this.childContainer.add_child(
                this.windowSimList[this.displayed + index]
            );
        }

        this.translationHelper.connect('transition-completed', () => {
            this.childContainer.remove_all_children();
            const actor1 = this.windowSimList[this.displayed];
            actor1.x = 0;
            this.childContainer.add_child(actor1);

            for (let index = 1; index < number_displayed; index++) {
                const actor = this.windowSimList[this.displayed + index];
                actor.x = 100 * index;
                this.childContainer.add_child(actor);
            }
        });

        const firstButton = new St.Button({
            style: 'background-color: rgba(0,0,200,0.5)',
            width: 30,
            child: new St.Label({ text: '0' }),
        });

        firstButton.connect('clicked', () => {
            if (this.displayed - 1 > -1) {
                this.translateTo(0);
            }
        });

        const leftButton = new St.Button({
            child: new St.Icon({
        ...
        leftButton.connect('clicked', () => {
            if (this.displayed - 1 > -1) {
                this.translateTo(this.displayed - 1);
            }
        });

        const workspaceSim = new St.Widget({
            layout_manager: new Clutter.BinLayout(),
            style: 'background-color: rgba(0,0,100,0.5)',
            width: 200,
            height: 150,
        ...
        workspaceSim.add_child(this.childContainer);

        const rightButton = new St.Button({
            child: new St.Icon({
                gicon: Gio.icon_new_for_string(
        ...
        rightButton.connect('clicked', () => {
            if (this.windowSimList.length > this.displayed + 1) {
                this.translateTo(this.displayed + 1);
            }
        });

        const lastButton = new St.Button({
            style: 'background-color: rgba(0,200,0,0.5)',
            width: 30,
            child: new St.Label({ text: 'last' }),
        });

        lastButton.connect('clicked', () => {
            this.translateTo(this.windowSimList.length - number_displayed);
        });
        this.add_child(firstButton);

        this.add_child(leftButton);
        this.add_child(workspaceSim);
        this.add_child(rightButton);
        this.add_child(lastButton);

        this.vertical = vertical;
    }

    translateTo(index: number) {
        this.translationHelper.setTranslation(
            [...this.windowSimList].splice(index, number_displayed),
            this.windowSimList,
            this.displayed > index ? -1 : 1
        );
