...
/** Weighted matching.
 * Matches N items on the "left" side with M items on the "right" side such that every item is part of at most one match, and the total cost is minimized.
 * 
 ...
 */
export function hungarian(costs: number[][]): { cost: number, assignments: Int32Array } {
...
export function test_hungarian() {
	log("TEST1");
    assert(hungarian([[10, 20, 30], [20, 30, 30], [20, 20, 5]]).cost == 45, "");
	log("TEST2");
	assert(hungarian([[10, 20, 10000], [1, 2, 10000], [5, 3, 10000]]).cost == 10000 + 4, "");
	log("PRINT");
	log(`HUNGARIAN: ${JSON.stringify(Array.from(hungarian([[10, 20, 10000], [1, 2, 10000], [5, 3, 10000]]).assignments))}`);
}
